...
if [ "$TARGET_ONLY" = "FALSE" ]; then # {
	...
	/sbin/insmod $IMAGEPATH/ar6000.ko bmienable=1 $ar6000args busspeedlow=$slowbus nohifscattersupport=1

	if [ $? -ne 0 ]; then
		echo "Failed to insert the module ar6000.ko"
		exit -1
	fi
fi # }
...
if [ "$bmi_enable" != "yes" ]; then # {
    ...
    if [ "$TARGET_VERSION" = "$AR6003_VERSION_REV2" -o  "$TARGET_VERSION" = "$AR6003_VERSION_REV3" ]; then
        ...
        $IMAGEPATH/bmiloader -i $NETIF --quiet --set --address=0x1404c --param=0x28
    ...
    if [ "$RAMFW" = "" ]; then # {
        ...
        # Part 1: EEPROM {
        if [ "$TARGET_VERSION" = "$AR6003_VERSION_REV2" -o  "$TARGET_VERSION" = "$AR6003_VERSION_REV3" ]; then
            EEPROM=${EEPROM-$WORKAREA/host/support/calData_ar6233_golden_bin_12132011.bin}
        fi
        if [ "$TARGET_TYPE" = "MCKINLEY" ]; then
        ...
        if [ "$TARGET_VERSION" = "$AR6003_VERSION_REV2" ]; then
            ...
            LOADADDR=0x543180
            BEGINADDR_BAK=$BEGINADDR
            BEGINADDR=0x944c00
        fi
        if [ "$TARGET_VERSION" = "$AR6003_VERSION_REV3" ]; then
            OTP=${OTP-$WORKAREA/otp.bin}
            LOADADDR=""
            BEGINADDR="0x945d20"
        fi
        if [ "$TARGET_TYPE" = "MCKINLEY" ]; then
        ...
        if [ "$TARGET_VERSION" = "$AR6003_VERSION_REV3" ]; then
            dsetimg=${dsetimg-$WORKAREA/data.patch.hw3_0.bin}
            LOADADDR=0x57fec8
        fi
        if [ "$TARGET_TYPE" = "MCKINLEY" ]; then
            dsetimg="" # TBD
