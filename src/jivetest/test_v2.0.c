...
//===================================================================
int GETRTC_Time(int fd);
int SETRTC_Time(int fd, int year, int mon, int day, int hour, int min, int sec );
...
int main(void)
{
	...
	int fd_KeyBacklight, fd_LcdnReset, fd_LcdBacklightEn, fd_MuteFET, fd_NANDnWP;
	int fd_rtc;
	...
	unsigned char keyback_data = 0xff;
...
//	fd_hold = open("/dev/HOLD", O_RDWR);
	...
	//--추가
	fd_KeyBacklight = open("/dev/KeyBacklightDIMM", O_RDWR );
	fd_LcdnReset = open("/dev/LcdReset", O_RDWR );
	fd_LcdBacklightEn = open("/dev/LcdBacklightEn", O_RDWR );
	fd_MuteFET = open("/dev/MuteFET", O_RDWR );
	fd_NANDnWP = open("/dev/NANDWP", O_RDWR );
...
//	if (fd_hold<0) {perror("/dev/HOLD"); exit(-1); }
	...
	//--추가
	if (fd_KeyBacklight <0) {perror("/dev/KeyBacklightDIMM"); exit(-1); }
	if (fd_LcdnReset <0) {perror("/dev/LcdReset"); exit(-1); }
	if (fd_LcdBacklightEn <0) {perror("/dev/LcdBacklightEn"); exit(-1); }
	if (fd_MuteFET <0) {perror("/dev/MuteFET"); exit(-1); }
	if (fd_NANDnWP <0) {perror("/dev/NANDWP"); exit(-1); }
			...
			case PWMTestMenu:
				...
				while (TestTemp)
				{
					switch (_PWM_Test_Str_menu())
					{
						...
						case KeyBacklight_Control:
						{
							TestTemp=1;
							while(TestTemp)
							{
								switch (_PWM_Key_Back_Str_menu())
								{
									case '1':
										if (keyback_data != 0xff) keyback_data+=0x10;
										buffer[0] = keyback_data;
										write(fd_KeyBacklight, buffer, 1);
										printf("keypad backlight Data : 0x%x\n",buffer[0]);
										break;
									case '2':
										if (keyback_data != 0x00) keyback_data-=0x10;
										buffer[0] = keyback_data;
										write(fd_KeyBacklight, buffer, 1);
										printf("keypad backlight Data : 0x%x\n",buffer[0]);
										break;
			...
			case GPIOTestMenu:
				...
				while (TestTemp)
				{
					switch (_GPIO_Test_Str_menu())
					{
							...
							while(TestTemp)
							{
								switch (_GPIO_HeadphoneDetection_Str_menu())
								{
									case PortDriveRead:
										...
										if (buffer[0] == 0x01) printf("Headphone plugged\n");
						...
						break;
						case Lcd_Backlight_En_Control:
							TestTemp=1;
							while(TestTemp)
							{
								switch (_GPIO_Lcd_Backlight_En_menu())
								{
									case PortDriveOff:
										printf("Lcd Backlight Disable\n");
										buffer[0] = 0x00;
										write(fd_LcdBacklightEn, buffer, 1);
										break;
									case PortDriveOn:
						...
						break;
						case Lcd_nReset_Control:
							TestTemp=1;
							while(TestTemp)
							{
								switch (_GPIO_Lcd_nReset_Str_menu())
								{
									case PortDriveOff:		//Low Active
										printf("Lcd nReset Low\n");
										buffer[0] = 0x00;
										write(fd_LcdnReset, buffer, 1);
										break;
									case PortDriveOn:
										printf("Lcd nReset High\n");
										buffer[0] = 0x01;
										write(fd_LcdnReset, buffer, 1);
										break;
									case BackPosition:
						...
						break;
						case Mute_FET_Control:
							TestTemp=1;
							while(TestTemp)
							{
								switch (_GPIO_Mute_FET_Control_Str_menu())
								{
									case PortDriveOff:		//Low active
										printf("Mute FET Enable(Low Active)\n");
										buffer[0] = 0x00;
										write(fd_MuteFET, buffer, 1);
										break;
									case PortDriveOn:
										printf("Mute FET Disable\n");
										buffer[0] = 0x01;
										write(fd_MuteFET, buffer, 1);
										break;
									case BackPosition:
						...
						break;
						case NAND_nWP_Control:
							TestTemp=1;
							while(TestTemp)
							{
								switch (_GPIO_NAND_nWP_Str_menu())
								{
									case PortDriveOff:		//Low Active
										printf("NAND is protected\n");
										buffer[0] = 0x00;
										write(fd_NANDnWP, buffer, 1);
										break;
									case PortDriveOn:
										printf("NAND is not protected\n");
										buffer[0] = 0x01;
										write(fd_NANDnWP, buffer, 1);
										break;
									case BackPosition:
				...
				while (TestTemp)
				{
					switch (_RTC_Test_Str_menu())
					{
						case SETRTC:
							...
							rtc1=(rtc1-48)*10;		rtc2=(rtc2-48);
							Tsec=rtc1+rtc2;
							printf(" Set Data [%d/%d/%d, %d:%d:%d] Complete \n",Tyear,Tmon,Tday,Thour,Tmin,Tsec);

   							SETRTC_Time(fd_rtc, Tyear, Tmon, Tday, Thour, Tmin, Tsec );
							GETRTC_Time(fd_rtc );
							break;
...
int GETRTC_Time( int fd )
{
	...
	if ( ioctl ( fd , RTC_RD_TIME, &rtc_time_data ) < 0 ) {
...
//	if( rtc < 0 ) {perror( "/dev/rtc open error" ); return -1;}
	if ( ioctl ( fd , RTC_SET_TIME, &rtc_time_data ) < 0 ) {
...
int _menuTest(void)
{
        ...
        printf("\n");
        printf("--------------------------------------------------------------------------\n");
        printf(" [1]. [Key]  -MATRIX / Scroll Wheel \n");
        printf(" [2]. [ADC]  -BATTERY Check\n");
        printf(" [3]. [PWM]  -LCD/Key Backlight \n");
        printf(" [4]. [GPIO]  -Headphone Detection / Charger Detection/ LCD / DAC\n");
        printf("             -AMP / WLAN Power /Mute TR / Motion Sensor / Mute FET / NAND nWP\n");
        printf(" [5]. [TFT LCD]  -16bpp\n");
        printf(" [6]. [IR]  -Tx / Rx\n");
        printf(" [7]. [POWER]  -Power mode Management\n");
        printf(" [8]. [RTC]  -S3C2412 Management  \n");
...
int _PWM_Test_Str_menu(void)
{
	 ...
	 printf("    2. Key backlight  control\n");
	 printf(" [B]. Back\n");
        printf(" => User Test Item Selection number ? ");
	 c= getchar();
	getchar();
...
int _PWM_Key_Back_Str_menu(void)
{
        int c;
	printf("\n");
       printf("--------------------------------------------------------------------------\n");
       printf(" [3]. [PWM]  Test  2. Key backlight control\n");
	printf("    Read = 0x00 ~ 0xFF\n");
	printf("    	1. Increase\n");
	printf("    	2. Decrease\n");
	printf(" [B]. Back\n");
...
int _GPIO_Test_Str_menu(void)
{
        ...
        printf("--------------------------------------------------------------------------\n");
        printf(" [4]. [GPIO]  Test\n");
	    printf("    1. Headphone Detection\n");
        printf("    2. LCD Power Control\n");
        printf("    3. DAC Power Control\n");
        printf("    4. AMP Power Control\n");
        printf("    5. Mute TR Control\n");
        printf("    6. Wlan Power Control\n");
        printf("    7. Motion Sensor\n");
        printf("    8. Lcd Backlight En Control\n");
        printf("    9. Lcd nReset Control\n");
        printf("    A. Mute FET Control\n");
        printf("    C. NAND nWP Control\n");
        printf(" [B]. Back\n");
        printf(" => User Test Item Selection number ? ");
	 c= getchar();
	getchar();
...
int _GPIO_HeadphoneDetection_Str_menu(void)
{
	...
	printf("\n");
       printf("--------------------------------------------------------------------------\n");
       printf(" [4]. [GPIO]  Test  1. Headphone Detection\n");
	printf("    Read = LOW(Command=0x00/Nothing) /  HIGH(Command=0x01/Check)\n");
	printf("    	1. Read\n");
	printf(" [B]. Back\n");
	printf(" => [1] Selection number ? ");
...
int _GPIO_LcdPower_Str_menu(void)
{
	...
	printf("\n");
	printf("--------------------------------------------------------------------------\n");
	printf(" [4]. [GPIO]  Test, 2. LCD Power Control\n");
...
int _GPIO_DACPower_Str_menu(void)
{
	...
	printf("\n");
	printf("--------------------------------------------------------------------------\n");
	printf(" [4]. [GPIO]  Test  3. DAC Power Control\n");
...
int _GPIO_AMPPower_Str_menu(void)
{
	...
	printf("\n");
	printf("--------------------------------------------------------------------------\n");
	printf(" [4]. [GPIO]  Test  4. AMP Power Control\n");
...
int _GPIO_MuteTR_Str_menu(void)
{
	...
	printf("\n");
	printf("--------------------------------------------------------------------------\n");
	printf(" [4]. [GPIO]  Test  5. Mute TR Control\n");
...
int _GPIO_WlanPower_Str_menu(void)
{
	...
	printf("\n");
	printf("--------------------------------------------------------------------------\n");
	printf(" [4]. [GPIO]  Test  6. Wlan Power Control\n");
	printf("   Wlan Power Control LOW(Command=0x00) / HIGH(Command=0x01)\n");
	printf("    	1. Wlan Power Control Off \n");
	printf("    	2. Wlan Power Control On \n");
	printf(" [B]. Back\n");
	printf(" => [1] or [2] User Test Selection number ? ");
	 c= getchar();
	getchar();
...
int _GPIO_MotionSensor_Str_menu(void)
{
	...
	printf("\n");
	printf("--------------------------------------------------------------------------\n");
	printf(" [4]. [GPIO]  Test 7. Motion Sensor\n");
	printf("    Read = LOW(Command=0x00/Nothing) /  HIGH(Command=0x01/Check)\n");
	printf("    	1. Read\n");
	printf(" [B]. Back\n");
	printf(" => [1] Selection number ? ");
...
int	_GPIO_Lcd_Backlight_En_menu(void)
{
        int c;
	printf("\n");
	printf("--------------------------------------------------------------------------\n");
	printf(" [4]. [GPIO]  Test  8. Lcd Backlight Enable Control\n");
	printf("   Lcd Backlight Enable Control LOW(Command=0x00) / HIGH(Command=0x01)\n");
	printf("    	1. Lcd Backlight En Off  \n");
	printf("    	2. Lcd Backlight En On \n");
	printf(" [B]. Back\n");
	printf(" => [1] or [2] User Test Selection number ? ");
	 c= getchar();
	getchar();
...
int	_GPIO_Lcd_nReset_Str_menu(void)
{
        int c;
	printf("\n");
	printf("--------------------------------------------------------------------------\n");
	printf(" [4]. [GPIO]  Test  9. Lcd nReset Control\n");
	printf("   Lcd nReset Control LOW(Command=0x00) / HIGH(Command=0x01)\n");
	printf("    	1. Lcd nReset Control Low \n");
	printf("    	2. Lcd nReset Control High \n");
	printf(" [B]. Back\n");
	printf(" => [1] or [2] User Test Selection number ? ");
	 c= getchar();
	getchar();
...
int	_GPIO_Mute_FET_Control_Str_menu(void)
{
        int c;
	printf("\n");
	printf("--------------------------------------------------------------------------\n");
	printf(" [4]. [GPIO]  Test  A. Mute FET Control\n");
	printf("   Mute FET Control LOW(Command=0x00) / HIGH(Command=0x01)\n");
	printf("    	1. Mute FET Control On(Mute) \n");
	printf("    	2. Mute FET Control Off(not Mute) \n");
	printf(" [B]. Back\n");
	printf(" => [1] or [2] User Test Selection number ? ");
	 c= getchar();
	getchar();
...
int	_GPIO_NAND_nWP_Str_menu(void)
{
        int c;
	printf("\n");
	printf("--------------------------------------------------------------------------\n");
	printf(" [4]. [GPIO]  Test  C. NAND nWP Control\n");
	printf("   NAND nWP Control LOW(Command=0x00) / HIGH(Command=0x01)\n");
	printf("    	1. NAND nWP Control On \n");
	printf("    	2. NAND nWP Control Off \n");
	printf(" [B]. Back\n");
	printf(" => [1] or [2] User Test Selection number ? ");
	 c= getchar();
	getchar();
