...
/* wjluv add this for booting kernel */
...
static inline void cache_clean_invalidate(void)
{
__asm__(
        "       mov     r1, #0\n"
        "       mov     r1, #7 << 5\n"            /* 8 segments */
...
{
/* change this for booting SMDK24A0 kernel */
#if 0
	ulong	iflag;
	ulong	addr;
...
# error Unknown CPU type
	...
	eth_halt();
        /* Check whether this image is linux kernel or not */
        if (*(ulong *)(CFG_KERNEL_BOOT + 9*4) != LINUX_ZIMAGE_MAGIC) {
                printf("Warning: this binary is not compressed linux kernel image\n");
                printf("zImage magic = 0x%08lx\n", *(ulong *)(CFG_KERNEL_BOOT + 9*4));
        } else {
                printf("zImage magic = 0x%08lx\n", *(ulong *)(CFG_KERNEL_BOOT + 9*4));
        }
