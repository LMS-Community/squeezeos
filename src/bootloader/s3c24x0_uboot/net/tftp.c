...
#define TFTP_OVER_32M  /* add define for Big size image download */
...
#ifdef TFTP_OVER_32M
static unsigned TftpBlock_Bigsize = 0;
#endif
#define STATE_RRQ	1
#define STATE_DATA	2
...
store_block (unsigned block, uchar * src, unsigned len)
{
	...
	uchar *destptr;
	destptr = load_addr + offset;
	adjust_bank1(destptr);
	(void)memcpy((void *)(destptr), src, len);
#else
...
#ifdef TFTP_OVER_32M
		if (TftpBlock == 0)
			TftpBlock_Bigsize = 0x10000;

		store_block (TftpBlock - 1 + TftpBlock_Bigsize, pkt + 2, len);
#else
