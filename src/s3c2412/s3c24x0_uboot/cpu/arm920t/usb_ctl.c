...
int usbctl_init( void )
{
        ...
        CLKCON |= CLKCON_USBD;
#if 0
        UPLLCON = FInsrt(60, fPLL_MDIV) | FInsrt(4, fPLL_PDIV)
                | FInsrt(2, fPLL_SDIV);
#endif
#if 0
        printf("EXNERNAL FIN  (%8d ) 0x%08X  \n",CONFIG_SYS_CLK_FREQ,CONFIG_SYS_CLK_FREQ);
        printf("UPLLCON 0x%08X CLKDIVN 0x%08X CAMDIVN 0x%08X  MISCCR 0x%08X\n",
                 UPLLCON,CLKDIVN,CAMDIVN, MISCCR);
#endif
                                                                                                                                                      
        /*  interrupt enable */
        INTMSK &= ~(BIT_USBD);
#if 0
        printf("INTMSK 0x%08X CLKSLOW 0x%08X \n", INTMSK, CLKSLOW);
#endif
