...
void ep1_int_hndlr(int udcsr)
{
	...
	int status;
	int recv_cnt;
	static int bank_switch_count=0;
	if (DNW) {
	...
	if (status & UD_OCSR1_PKTRDY) {
		...
		if (DNW) {
			...
			#if defined(OREIAS) || defined(CONFIG_DISCONTIGMEM)
				u8 *tptr;
				tptr = (usbd_dn_addr+usbd_dn_cnt);
				adjust_bank1(tptr);
		...
		if(!last_packet){
			...
			upgrade_status = 1;
			upgrade_size = (ulong)usbd_dn_cnt;
                   }
