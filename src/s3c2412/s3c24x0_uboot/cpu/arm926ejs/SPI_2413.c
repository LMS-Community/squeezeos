...
{
	// GPG9
	rGPGDN |= (0x1<<9);			// Pull-down Disable

	rGPGCON &= ~(0x3<<18);	//output setting
	rGPGCON |= (0x1<<18);		//

	rGPGDAT |= (1<<9);			// nRESET = 'High'
}

void SPI_nRESET_Con(int value)
{
	rGPGDAT=(rGPGDAT&~(1<<9))|(value<<9);
...
{
	SPI_Port_Init(0);
	SPI_Baud_Rate_Set(0, 14000000);			// SPI Channel 0~10MHz
