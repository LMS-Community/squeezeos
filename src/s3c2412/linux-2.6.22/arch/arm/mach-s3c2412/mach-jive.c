...
static int __init jive_mtdset(char *options)
{
	...
	set = simple_strtoul(options, NULL, 10);
...
struct s3c2410fb_display jive_vgg2432a4_display[] = {
	[0] = {
		...
		.yres_virtual	= LCD_YRES * 2, // for hardware double buffer
...
static void jive_backlight_powercb(struct device *dev,
				   unsigned int pre, unsigned int to)
{
	if (pre == 1 && to == 0) {
...
static struct spi_board_info jive_wm8750_board[] = {
	[0] = {
		.modalias	= "WM8750",
		.chip_select	= 0,
...
static void __init jive_machine_init(void)
{
	...
	__raw_writel(S3C2412_SLPCON_PULL(0) |
		     ...
		     S3C2412_SLPCON_IN(15), S3C2412_GPESLPCON);
...
#ifdef CONFIG_MACH_JIVE_OLDID
MACHINE_START(S3C2413, "S3C2413_JIVE")
	/* Maintainer: Ben Dooks <ben@fluff.org> */
	.phys_io	= S3C2410_PA_UART,
	.io_pg_offst	= (((u32)S3C24XX_VA_UART) >> 18) & 0xfffc,
	.boot_params	= S3C2410_SDRAM_PA + 0x100,
