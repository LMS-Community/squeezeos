...
int wm8750_init(struct snd_soc_device *socdev)
{
	...
	struct wm8750_setup_reg *regs = setup->regs;
	int reg, ret = 0;
	...
	if (regs != NULL && setup->regs_size > 0) {
		for (reg = 0; reg < setup->regs_size; reg++, regs++) {
			wm8750_write(codec, regs->reg, regs->val);
		}
	}
...
EXPORT_SYMBOL_GPL(wm8750_probe);
