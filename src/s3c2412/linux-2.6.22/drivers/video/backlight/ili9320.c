...
static inline void ili9320_power_cb(struct ili9320 *lcd, int pre, int pwr)
{
	if (lcd->platdata->set_power)
		(lcd->platdata->set_power)(lcd->dev, pre, pwr);
}

static inline int ili9320_power_on(struct ili9320 *lcd)
{
	ili9320_power_cb(lcd, 1, 1);
...
{
	ili9320_power_cb(lcd, 1, 0);

	lcd->display1 &= ~(ILI9320_DISPLAY1_D(3) | ILI9320_DISPLAY1_BASEE);
	ili9320_write(lcd, ILI9320_DISPLAY1, lcd->display1);

	ili9320_power_cb(lcd, 0, 0);
...
static inline struct ili9320 *lcd_get_data(struct lcd_device *lcd)
{
	return class_get_devdata(&lcd->class_dev);
}

static int ili9320_set_power(struct lcd_device *ld, int power)
{
	struct ili9320 *lcd = lcd_get_data(ld);
				...
				struct ili9320_client *client)
{
	...
	if (cfg->hsize <= 0 || cfg->vsize <= 0 || cfg->reset == NULL) {
		dev_err(&spi->dev, "invalid platform data supplied\n");
		return -EINVAL;
	}
	...
	lcd = lcd_device_register("ili9320", ili, &ili9320_ops);
	if (IS_ERR(lcd)) {
		dev_err(&spi->dev, "failed to register lcd device\n");
		ret = PTR_ERR(lcd);
		goto err_free;
	}
	...
	dev_info(ili->dev, "initialising %s\n", client->name);

	ret = ili9320_power(ili, FB_BLANK_UNBLANK);
	if (ret != 0) {
		dev_err(&spi->dev, "failed to set lcd power state\n");
		goto err_unregister;
	}
...
int ili9320_suspend(struct ili9320 *lcd, pm_message_t state)
{
	...
	if (state.event == PM_EVENT_SUSPEND) {
		...
		if (lcd->platdata->suspend == ILI9320_SUSPEND_DEEP) {
			ili9320_write(lcd, ILI9320_POWER1,
				      lcd->power1 | ILI9320_POWER1_SLP | ILI9320_POWER1_DSTB);
			lcd->initialised = 0;
		}
