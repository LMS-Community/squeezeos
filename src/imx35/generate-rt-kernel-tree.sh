...
if [ "$(basename $(pwd))" != "${KERNEL_BASE}-rt" ]; then
	die "Please, execute this script from the ${KERNEL_BASE}-rt directory"
fi
...
(cd ${BUILD_DIR}/${KERNEL_BASE} && quilt push -a) || die "Failed to apply kernel patches"
echo ""
echo "===================================================="
