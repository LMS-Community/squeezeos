/* $Header: /wwg/motif/xltwavplay/RCS/wavplay.h,v 1.1 1997/04/14 00:56:48 wwg Exp $
 ...
 */
#ifndef _wavplay_h_
#define _wavplay_h_ "@(#)wavplay.h $Revision: 1.1 $"

#define WAVPLAY_VERSION		"1.0"
...
typedef enum {
	...
	MSGTYP_Last=20,				/* This is not really a message type */
} MSGTYP;
...
typedef struct {
	...
	union	{
		 ...
		 * stat() error when errnox != 0, or stat() information
		 * when errnox == 0.
		 */
		struct	{
			int	errnox;		/* Zero if OK, else errnox from stat() */
			struct stat sbuf;	/* Path's stat info */
		} toclnt_stat;

		/*
		 * Message to X client, from server, indicating an
		 * error if errnox != 0, or successfully obtained
		 * WAV file info if errnox == 0.
		 */
		struct	{
			int	errnox;		/* Zero if OK, else errnox value */
...
extern WAVFILE *WavOpenForWrite(const char *Pathname,Chan m,UInt32 sample_rate,UInt16 bits,UInt32 samples,ErrFunc erf);
...
//extern char *sys_errlist[];			/* System list of error messages */

#endif /* _wavplay_h_ */

/* $Source: /wwg/motif/xltwavplay/RCS/wavplay.h,v $ */
