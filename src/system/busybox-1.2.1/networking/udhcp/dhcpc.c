...
/*
 * 29 May 2007 Richard applied the zeroconf patch from:
...
int udhcpc_main(int argc, char *argv[])
{
	...
	while (1) {
		int option_index = 0;
		c = getopt_long(argc, argv, "c:CV:fbH:h:F:i:znp:qr:s:T:t:v", arg_options, &option_index);
		if (c == -1) break;
	...
	for (;;) {
     		...
     		/* configures timeout for select(), the smaller between
		   DHCP and Zeroconf timeouts */
#ifdef ENABLE_FEATURE_UDHCP_ZEROCONF
