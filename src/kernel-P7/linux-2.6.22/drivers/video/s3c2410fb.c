			       ...
			       struct fb_info *info)
{
	...
	var->xres_virtual = display->xres_virtual;
	var->yres_virtual = display->yres_virtual;
...
{
	struct s3c2410fb_info *fbi = info->par;
	unsigned long saddr1, saddr2, val, flags;
	void __iomem *regs = fbi->io;

	saddr1 = info->fix.smem_start;
	saddr1 += var->xoffset;
	saddr1 += info->fix.line_length * var->yoffset;
	...
	do {
		val = readl(regs + S3C2410_LCDCON1) >> 18;
	} while (val == 0);

	writel(saddr1 >> 1, regs + S3C2410_LCDSADDR1);
	writel(saddr2 >> 1, regs + S3C2410_LCDSADDR2);

	local_irq_restore(flags);

	/* FIXME we should wait for the start of the next frame here but
	 * the Jive application process events after flipping the
	 * framebuffer, so by not delaying we save a few ms per frame.
	 */
				  ...
				  enum s3c_drv_type drv_type)
{
	...
	for (i = 0; i < mach_info->num_displays; i++) {
		...
		smem_len *= mach_info->displays[i].yres_virtual;
