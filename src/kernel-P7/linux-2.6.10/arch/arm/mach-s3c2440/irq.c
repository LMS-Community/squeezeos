...
s3c_irq_mask(unsigned int irqno)
{
	...
	mask = __raw_readl(S3C2440_INTMSK);
	mask |= 1UL << irqno;
	__raw_writel(mask, S3C2440_INTMSK);
}
...
s3c_irq_ack(unsigned int irqno)
{
	...
	__raw_writel(bitval, S3C2440_SRCPND);
	__raw_writel(bitval, S3C2440_INTPND);
}
...
s3c_irq_maskack(unsigned int irqno)
{
	...
	mask = __raw_readl(S3C2440_INTMSK);
	__raw_writel(mask|bitval, S3C2440_INTMSK);
...
s3c_irqext_mask(unsigned int irqno)
{
	...
	mask = __raw_readl(S3C2440_EINTMASK);
	mask |= ( 1UL << irqno);
	__raw_writel(mask, S3C2440_EINTMASK);
...
s3c_irqext_ack(unsigned int irqno)
{
	...
	__raw_writel(bit, S3C2440_EINTPEND);

	req = __raw_readl(S3C2440_EINTPEND);
	req &= ~mask;
...
s3c_irqext_type(unsigned int irq, unsigned int type)
{
	...
	{
		gpcon_reg = S3C2440_GPFCON;
		extint_reg = S3C2440_EXTINT0;
	...
	{
		gpcon_reg = S3C2440_GPGCON;
		extint_reg = S3C2440_EXTINT1;
		...
		int subcheck)
{
	...
	submask = __raw_readl(S3C2440_INTSUBMSK);
	mask = __raw_readl(S3C2440_INTMSK);
	...
	if ((submask  & subcheck) == subcheck) {
		__raw_writel(mask | parentbit, S3C2440_INTMSK);
	}

	/* write back masks */
	__raw_writel(submask, S3C2440_INTSUBMSK);
...
s3c_irqsub_maskack(unsigned int irqno, unsigned int parentmask, unsigned int group)
{
	...
	__raw_writel(bit, S3C2440_SUBSRCPND);
	...
	if (1) {
		__raw_writel(parentmask, S3C2440_SRCPND);
		__raw_writel(parentmask, S3C2440_INTPND);
	}
}
			      ...
			      struct pt_regs *regs)
{
	...
	subsrc = __raw_readl(S3C2440_SUBSRCPND);
	submsk = __raw_readl(S3C2440_INTSUBMSK);
...
void __init s3c24xx_init_irq(void)
{
	...
	irqdbf("s3c2440_init_irq: clearing interrupt status flags\n");

	/* first, clear all interrupts pending... */

	last = 0;
	for (i = 0; i < 4; i++) {
		pend = __raw_readl(S3C2440_EINTPEND);

		if (pend == 0 || pend == last)
			break;

		__raw_writel(pend, S3C2440_EINTPEND);
	...
	last = 0;
	for (i = 0; i < 4; i++) {
		pend = __raw_readl(S3C2440_INTPND);

		if (pend == 0 || pend == last)
			break;

		__raw_writel(pend, S3C2440_SRCPND);
		__raw_writel(pend, S3C2440_INTPND);
	...
	last = 0;
	for (i = 0; i < 4; i++) {
		pend = __raw_readl(S3C2440_SUBSRCPND);

		if (pend == 0 || pend == last)
			break;

		printk("irq: clearing subpending status %08x\n", (int)pend);
		__raw_writel(pend, S3C2440_SUBSRCPND);
		last = pend;
	}

	/* register the main interrupts */

	irqdbf("s3c2440_init_irq: registering s3c2440 interrupt handlers\n");

	for (irqno = IRQ_BATT_FLT; irqno <= IRQ_ADCPARENT; irqno++) {
		/* set all the s3c2440 internal irqs */
	...
	irqdbf("s3c2440: registering external interrupts\n");
	...
	irqdbf("s3c2440: registered interrupt handlers\n");
}
