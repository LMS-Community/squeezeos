...
static struct resource s3c_uart0_resource[] = {
	[0] = {
		.start = S3C2440_PA_UART0,
		.end   = S3C2440_PA_UART0 + 0x3fff,
...
static struct resource s3c_uart1_resource[] = {
	[0] = {
		.start = S3C2440_PA_UART1,
		.end   = S3C2440_PA_UART1 + 0x3fff,
		.flags = IORESOURCE_MEM,
	},
	[1] = {
...
static struct resource s3c_uart2_resource[] = {
	[0] = {
		.start = S3C2440_PA_UART2,
		.end   = S3C2440_PA_UART2 + 0x3fff,
		.flags = IORESOURCE_MEM,
	},
	[1] = {
...
void __init s3c2440_init_uarts(struct s3c2440_uartcfg *cfg, int no)
{
...
{
	s3c2440_pm_do_save(s3c2440_sleep, ARRAY_SIZE(s3c2440_sleep));
	return 0;
}

static int s3c2440_resume(struct sys_device *dev)
{
	s3c2440_pm_do_restore(s3c2440_sleep, ARRAY_SIZE(s3c2440_sleep));
	return 0;
}
...
void __init s3c2440_map_io(struct map_desc *mach_desc, int size)
{
	...
	/* rename any peripherals used differing from the s3c2440 */
...
void __init s3c2440_init_clocks(int xtal)
{
	...
	s3c24xx_fclk = s3c2440_get_pll(__raw_readl(S3C2440_MPLLCON),
				       s3c24xx_xtal)*2 ;

	clkdiv = __raw_readl(S3C2440_CLKDIVN);
