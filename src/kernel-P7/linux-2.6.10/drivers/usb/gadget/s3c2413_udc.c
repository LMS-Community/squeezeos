...
#include "s3c2413_udc.h"

#define GPIO_INIT	do{						\
        			u32 val;				\
				val = __raw_readl(S3C2413_GPFCON);	\
			        val &= ~(3<<4);				\
			        val |= (1<<4);				\
			        __raw_writel(val, S3C2413_GPFCON);	\
				val = __raw_readl(S3C2413_GPFDN);	\
				val |= (1<<2);				\
				__raw_writel(val, S3C2413_GPFDN);	\
				val = __raw_readl(S3C2413_GPFDAT);	\
				val &= ~(1<<2);				\
				__raw_writel(val, S3C2413_GPFDAT);	\
			}while(0)
...
static const char driver_name[] = "s3c2413_udc";
...
{
	__raw_writel(ind, S3C2413_UDC_INDEX_REG);
...
static void udc_reinit(struct elfin_udc *dev)
{
	...
	for (i = 0; i < S3C2413_MAX_ENDPOINTS; i++) {
...
static int udc_enable(struct elfin_udc *dev)
{
	...
	val = usb_read(S3C2413_MISCCR, 0);
	val &= ~((1<<3)|(1<<13));       // USB Device in Normal mode
	usb_write(val, S3C2413_MISCCR, 0);
	...
	val = (1<<0);
	usb_write(val, S3C2413_UDC_EP_INT_REG, 0);	//clear
	val = (1<<2);   //UD_USBINTE_RESET
	usb_write(val, S3C2413_UDC_USB_INT_REG, 0);	//clear

	/* Set MAXP values for each */
	usb_write(dev->ep[0].ep.maxpacket>>3, S3C2413_UDC_MAXP_REG, 0);

	usb_write(dev->ep[1].ep.maxpacket>>3, S3C2413_UDC_MAXP_REG, 1);

	usb_write(dev->ep[2].ep.maxpacket>>3, S3C2413_UDC_MAXP_REG, 2);

	usb_write(dev->ep[3].ep.maxpacket>>3, S3C2413_UDC_MAXP_REG, 3);

	usb_write(dev->ep[4].ep.maxpacket>>3, S3C2413_UDC_MAXP_REG, 4);
...
static int write_fifo(struct elfin_ep *ep, struct elfin_request *req)
{
	...
	if (is_last) {
		if(!ep_index(ep)){	// EP0 must not come here
			BUG();
			//usb_set(S3C2413_UDC_EP0_CSR_IPKRDY | S3C2413_UDC_EP0_CSR_DE, 
			//		S3C2413_UDC_EP0_CSR_REG, 0);
		}else{
			usb_set(S3C2413_UDC_ICSR1_PKTRDY, ep->csr1, ep_index(ep));
		}
		done(ep, req, 0);
		return 1;
	} else{
...
static int read_fifo(struct elfin_ep *ep, struct elfin_request *req)
{
	...
	if (!(csr & S3C2413_UDC_OCSR1_PKTRDY)) {
	...
	count = (( (usb_read(S3C2413_UDC_OUT_FIFO_CNT2_REG, ep_index(ep)) & 0xff ) << 8) | (usb_read(S3C2413_UDC_OUT_FIFO_CNT1_REG, ep_index(ep)) & 0xff));
	...
	usb_clear(S3C2413_UDC_OCSR1_PKTRDY, ep->csr1, ep_index(ep));
...
static void elfin_in_epn(struct elfin_udc *dev, u32 ep_idx)
{
	...
	if (csr & S3C2413_UDC_ICSR1_SENTSTL) {
		printk("S3C2413_UDC_ICSR1_SENTSTL\n");
		usb_set(S3C2413_UDC_ICSR1_SENTSTL /*| S3C2413_UDC_ICSR1_SENDSTL */ ,
			ep->csr1, ep_index(ep));
		return;
	}
...
static void elfin_out_epn(struct elfin_udc *dev, u32 ep_idx)
{
	...
	if (ep->desc) {
		...
		while ((csr = usb_read(ep->csr1, ep_index(ep))) 
				& (S3C2413_UDC_OCSR1_PKTRDY | S3C2413_UDC_OCSR1_SENTSTL)) {

			DEBUG("%s: %x\n", __FUNCTION__, csr);

			if (csr & S3C2413_UDC_OCSR1_SENTSTL) {
				DEBUG("%s: stall sent, flush fifo\n",
				      __FUNCTION__);
			/* usb_set(USB_OUT_CSR1_FIFO_FLUSH, ep->csr1, ep_index(ep)); */
			} else if (csr & S3C2413_UDC_OCSR1_PKTRDY) {
				if (list_empty(&ep->queue))
					req = 0;
				else
...
static void reconfig_usbd(void)
{
	...
	usb_write(0, S3C2413_UDC_PWR_REG, 0);

	/* EP0 */
	val = EP0_FIFO_SIZE>>3;
	usb_write(val, S3C2413_UDC_MAXP_REG, 0);

	val = (1<<6)|(1<<7);	//EP0_CSR_SOPKTRDY | EP0_CSR_SSE;
	usb_write(val, S3C2413_UDC_EP0_CSR_REG, 0);

	/* EP3 -- INT */
	val = EP_FIFO_SIZE>>3;
	usb_write(val, S3C2413_UDC_MAXP_REG, 3);

	val = (1<<6)|(1<<3);	//UD_ICSR1_FFLUSH | UD_ICSR1_CLRDT;
	usb_write(val, S3C2413_UDC_IN_CSR1_REG, 3);

	val = (1<<5)|(1<<4);	//UD_ICSR2_MODEIN | UD_ICSR2_DMAIEN; 
	usb_write(val, S3C2413_UDC_IN_CSR2_REG, 3);

	/* EP2 -- IN */
	val = EP_FIFO_SIZE>>3;
	usb_write(val, S3C2413_UDC_MAXP_REG, 2);

	val = (1<<6)|(1<<3);	//UD_ICSR1_FFLUSH | UD_ICSR1_CLRDT;
	usb_write(val, S3C2413_UDC_IN_CSR1_REG, 2);

	val = (1<<5)|(1<<4);	//UD_ICSR2_MODEIN | UD_ICSR2_DMAIEN; 
	usb_write(val, S3C2413_UDC_IN_CSR2_REG, 2);

	/* EP1 -- OUT */
	val = EP_FIFO_SIZE>>3;
	usb_write(val, S3C2413_UDC_MAXP_REG, 1);

	val = (1<<6)|(1<<3);	//UD_ICSR1_FFLUSH | UD_ICSR1_CLRDT;
	usb_write(val, S3C2413_UDC_IN_CSR1_REG, 1);

	usb_write(0, S3C2413_UDC_IN_CSR2_REG, 1);	// output mode

	val = (1<<4)|(1<<7);	//UD_OCSR1_FFLUSH | UD_OCSR1_CLRDT;
	usb_write(val, S3C2413_UDC_OUT_CSR1_REG, 1);

	val = (1<<5);		//UD_OCSR2_DMAIEN;
	usb_write(val, S3C2413_UDC_OUT_CSR2_REG, 1);
	...
	usb_write(val, S3C2413_UDC_EP_INT_EN_REG, 0);
	usb_write(val & ~(1<<0), S3C2413_UDC_EP_INT_REG, 0);	//clear

	val = (1<<0)|(1<<2);	//UD_USBINTE_RESET | UD_USBINTE_SUSPND;
	usb_write(val, S3C2413_UDC_USB_INT_EN_REG, 0);
	usb_write(val, S3C2413_UDC_USB_INT_REG, 0);	//clear
}
...
static irqreturn_t elfin_udc_irq(int irq, void *_dev, struct pt_regs *r)
{
	...
	intr_out = intr_in = usb_read(S3C2413_UDC_EP_INT_REG, 0);
	intr_int = usb_read(S3C2413_UDC_USB_INT_REG, 0);

	/* We have only 3 usable eps now 	-jassi */
	intr_in &= (S3C2413_UDC_INT_EP3 | S3C2413_UDC_INT_EP2 | S3C2413_UDC_INT_EP0);
	intr_out &= S3C2413_UDC_INT_EP1;
	...
	if (intr_in) {
		if (intr_in & S3C2413_UDC_INT_EP0){
			//printk("EP0 ");
			usb_write(S3C2413_UDC_INT_EP0, S3C2413_UDC_EP_INT_REG, 0);
			elfin_handle_ep0(dev);
		}else if (intr_in & S3C2413_UDC_INT_EP2){
			//printk("EP2 ");
			usb_write(S3C2413_UDC_INT_EP2, S3C2413_UDC_EP_INT_REG, 2);
			elfin_in_epn(dev, 2);	// hard coded !!!   -jassi
		}else{
			//printk("EP3 ");
			usb_write(S3C2413_UDC_INT_EP3, S3C2413_UDC_EP_INT_REG, 3);
	...
	if (intr_out) {
		//printk("EP1 ");
		usb_write(S3C2413_UDC_INT_EP1, S3C2413_UDC_EP_INT_REG, 1);
		elfin_out_epn(dev, 1);		// hard coded !!!   -jassi
	}

	if (intr_int) {
		if (intr_int & S3C2413_UDC_USBINT_RESET) {
			reconfig_usbd();
			dev->gadget.speed = USB_SPEED_FULL;
			dev->ep0state = WAIT_FOR_SETUP;
			usb_write(S3C2413_UDC_USBINT_RESET, S3C2413_UDC_USB_INT_REG, 0);
			DEBUG_SETUP("RESET");
		}

		usb_write(intr_int, S3C2413_UDC_USB_INT_REG, 0);

		if (intr_int & S3C2413_UDC_USBINT_RESUME) {
		...
		if (intr_int & S3C2413_UDC_USBINT_SUSPEND) {
			if (dev->gadget.speed !=
			 ...
			 int gfp_flags)
{
	...
	if (list_empty(&ep->queue) && likely(!ep->stopped)) {
		...
		if (unlikely(ep_index(ep) == 0)) {
			...
			csr = usb_read(ep->csr1, ep_index(ep));
			if(!(csr & S3C2413_UDC_ICSR1_PKTRDY)&&(write_fifo(ep, req) == 1))
				req = 0;
		} else {
			csr = usb_read(ep->csr1, ep_index(ep));
...
static int elfin_fifo_status(struct usb_ep *_ep)
{
	...
	if (ep->dev->gadget.speed != USB_SPEED_UNKNOWN ||
	    csr & S3C2413_UDC_OCSR1_PKTRDY) {
		count = ( ((usb_read(S3C2413_UDC_OUT_FIFO_CNT2_REG, ep_index(ep)) & 0xff) << 8) 
			| (usb_read(S3C2413_UDC_OUT_FIFO_CNT1_REG, ep_index(ep)) & 0xff));
	}
...
static int read_fifo_ep0(struct elfin_ep *ep, struct elfin_request *req)
{
	...
	csr = usb_read(S3C2413_UDC_EP0_CSR_REG, ep_index(ep));
	if (!(csr & S3C2413_UDC_OCSR1_PKTRDY))
		return 0;
	...
	/* read all bytes from this packet */
	if (likely(csr & S3C2413_UDC_EP0_CSR_OPKRDY)) {
		count = 8;	// I do !!!
		req->req.actual += min(count, bufferspace);
	} else	{		// zlp 
		count = 0;
...
static void udc_set_address(struct elfin_udc *dev, unsigned char address)
{
	...
	usb_set((1<<7) | (address & 0x7f), S3C2413_UDC_FUNC_ADDR_REG, 0);
}

/*
 * DATA_STATE_RECV (OUT_PKT_RDY)
 *      - if error
 *              set S3C2413_UDC_EP0_CSR_SOPKTRDY | S3C2413_UDC_EP0_CSR_DE | S3C2413_UDC_EP0_CSR_SENDSTL bits
 *      - else
 *              set S3C2413_UDC_EP0_CSR_SOPKTRDY bit
 				if last set S3C2413_UDC_EP0_CSR_DE bit
 */
...
static void elfin_ep0_read(struct elfin_udc *dev)
{
	...
	if(req->req.length == 0) {
		usb_set((S3C2413_UDC_EP0_CSR_SOPKTRDY | S3C2413_UDC_EP0_CSR_DE),
				 S3C2413_UDC_EP0_CSR_REG, 0);
		dev->ep0state = WAIT_FOR_SETUP;
		first_time = 1;
		done(ep, req, 0);
	...
	if(!req->req.actual && first_time){	//for SetUp packet
		usb_set(S3C2413_UDC_EP0_CSR_SOPKTRDY, S3C2413_UDC_EP0_CSR_REG, 0);
		first_time = 0;
		return;
	}
...
static int elfin_ep0_write(struct elfin_udc *dev)
{
	...
	if ((ret == 1) && !need_zlp) {
		...
		usb_set(S3C2413_UDC_EP0_CSR_IPKRDY, S3C2413_UDC_EP0_CSR_REG, 0);
	}
				     ...
				     struct usb_ctrlrequest *ctrl)
{
	...
	if (reqtype == USB_RECIP_INTERFACE) {
		...
		case ep_control:
			val =
			    (usb_read(qep->csr1, ep_index(qep)) & S3C2413_UDC_EP0_CSR_SENDSTL) ==
			    S3C2413_UDC_EP0_CSR_SENDSTL;
			break;
		case ep_bulk_in:
		case ep_interrupt:
			val = (usb_read(qep->csr1, ep_index(qep))
				 & S3C2413_UDC_ICSR1_SENDSTL) == S3C2413_UDC_ICSR1_SENDSTL;
			break;
		case ep_bulk_out:
			val = (usb_read(qep->csr1, ep_index(qep)) 
				& S3C2413_UDC_OCSR1_SENDSTL) == S3C2413_UDC_OCSR1_SENDSTL;
			break;
		}
 ...
 *              set S3C2413_UDC_EP0_CSR_SOPKTRDY | S3C2413_UDC_EP0_CSR_DE | S3C2413_UDC_EP0_CSR_SENDSTL bits
 *      - else
 *              set S3C2413_UDC_EP0_CSR_SOPKTRDY | S3C2413_UDC_EP0_CSR_DE bits
 */
static void elfin_ep0_setup(struct elfin_udc *dev, u32 csr)
{
	...
	case USB_REQ_SET_ADDRESS:
		...
		usb_set((S3C2413_UDC_EP0_CSR_SOPKTRDY | S3C2413_UDC_EP0_CSR_DE), S3C2413_UDC_EP0_CSR_REG, 0);
		return;
#if 0
	...
	if (likely(dev->driver)) {
		...
		if (i < 0) {
			...
			printk("  --> ERROR: gadget setup FAILED (stalling), \
				setup returned %d\n", i);
			usb_set((S3C2413_UDC_EP0_CSR_SOPKTRDY | S3C2413_UDC_EP0_CSR_DE 
				|S3C2413_UDC_EP0_CSR_SENDSTL), S3C2413_UDC_EP0_CSR_REG, 0);
			/* ep->stopped = 1; */
			dev->ep0state = WAIT_FOR_SETUP;
		}
	}
...
static void elfin_handle_ep0(struct elfin_udc *dev)
{
	 ...
	 */
	if (csr & S3C2413_UDC_EP0_CSR_SENTSTL) {
		printk("%s: S3C2413_UDC_EP0_CSR_SENTSTL is set: %x\n", __FUNCTION__, csr);
		usb_write(0, S3C2413_UDC_EP0_CSR_REG, 0);
	 ...
	 */
	if (csr & S3C2413_UDC_EP0_CSR_SE) {
		DEBUG_EP0("%s: S3C2413_UDC_EP0_CSR_SE is set: %x\n", __FUNCTION__, csr);

		usb_clear(0xc0, S3C2413_UDC_EP0_CSR_REG, 0);
		usb_set(S3C2413_UDC_EP0_CSR_SSE, S3C2413_UDC_EP0_CSR_REG, 0);
	 ...
	 *              set IN_PKT_RDY
	 */
	if (!(csr & (S3C2413_UDC_EP0_CSR_IPKRDY | S3C2413_UDC_EP0_CSR_OPKRDY))) {
		switch (dev->ep0state) {
		...
		default:
			...
			dev->ep0state = WAIT_FOR_SETUP;
			/* nuke(ep, 0); */
			/* usb_set(S3C2413_UDC_EP0_CSR_SENDSTL, ep->csr1, ep_index(ep)); */
			break;
		}
	}

	/*
	 * if S3C2413_UDC_EP0_CSR_OPKRDY is set
	 *      - read data packet from EP0 FIFO
	 *      - decode command
	 *      - if error
	 ...
	 */
	if (csr & S3C2413_UDC_EP0_CSR_OPKRDY) {
		switch (dev->ep0state) {
		case WAIT_FOR_SETUP:
...
{
	u32 frame1 = usb_read(S3C2413_UDC_FRAME_NUM1_REG, 0);/* Least significant 8 bits */
	u32 frame2 = usb_read(S3C2413_UDC_FRAME_NUM1_REG, 0);/* Most significant 3 bits */
...
static struct elfin_udc memory = {
	...
	.ep[0] = {
		  ...
		  .ep_type = ep_control,
		  .fifo = S3C2413_UDC_EP0_FIFO_REG,
		  .csr1 = S3C2413_UDC_EP0_CSR_REG,
		  .csr2 = S3C2413_UDC_EP0_CSR_REG,
		  },
	...
	.ep[1] = {
		  ...
		  .ep_type = ep_bulk_out,
		  .fifo = S3C2413_UDC_EP1_FIFO_REG,
		  .csr1 = S3C2413_UDC_OUT_CSR1_REG,
		  .csr2 = S3C2413_UDC_OUT_CSR2_REG,
		  },
	...
	.ep[2] = {
		  ...
		  .ep_type = ep_bulk_in,
		  .fifo = S3C2413_UDC_EP2_FIFO_REG,
		  .csr1 = S3C2413_UDC_IN_CSR1_REG,
		  .csr2 = S3C2413_UDC_IN_CSR2_REG,
		  },
