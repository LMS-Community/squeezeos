...
#define s3c24xx_dev_to_cfg(__dev) (struct s3c2440_uartcfg *)((__dev)->platform_data)
...
{
	return (rd_regl(port, S3C2440_UTRSTAT) & S3C2440_UTRSTAT_TXE);
}
...
static void s3c24xx_serial_rx_enable(struct uart_port *port)
{
	...
	ufcon = rd_regl(port, S3C2440_UFCON);
	ufcon |= S3C2440_UFCON_RESETRX;
	wr_regl(port, S3C2440_UFCON, ufcon);

	ucon = rd_regl(port, S3C2440_UCON);
	ucon |= S3C2440_UCON_RXIRQMODE;
	wr_regl(port, S3C2440_UCON, ucon);
...
static inline struct s3c2440_uartcfg *s3c24xx_port_to_cfg(struct uart_port *port)
{
	if (port->dev == NULL)
		return NULL;

	return (struct s3c2440_uartcfg *)port->dev->platform_data;
}
...
#define S3C2440_UERSTAT_PARITY (0x1000)
...
s3c24xx_serial_rx_chars(int irq, void *dev_id, struct pt_regs *regs)
{
	...
	while (max_count-- > 0) {
		ufcon = rd_regl(port, S3C2440_UFCON);
		ufstat = rd_regl(port, S3C2440_UFSTAT);
		...
		uerstat = rd_regl(port, S3C2440_UERSTAT);
		ch = rd_regb(port, S3C2440_URXH);
		...
		if (uerstat & S3C2440_UERSTAT_ANY) {
			dbg("rxerr: port ch=0x%02x, rxs=0x%08x\n",
			    ch, uerstat);

			/* check for break */
			if (uerstat & S3C2440_UERSTAT_BREAK) {
				dbg("break!\n");
				port->icount.brk++;
			...
			if (uerstat & S3C2440_UERSTAT_FRAME)
				port->icount.frame++;
			if (uerstat & S3C2440_UERSTAT_OVERRUN)
				port->icount.overrun++;
			...
			if (uerstat & S3C2440_UERSTAT_BREAK)
				flag = TTY_BREAK;
			else if (uerstat & S3C2440_UERSTAT_PARITY)
				flag = TTY_PARITY;
			else if (uerstat & ( S3C2440_UERSTAT_FRAME | S3C2440_UERSTAT_OVERRUN))
				flag = TTY_FRAME;
		}
...
static irqreturn_t s3c24xx_serial_tx_chars(int irq, void *id, struct pt_regs *regs)
{
	...
	if (port->x_char) {
		wr_regb(port, S3C2440_UTXH, port->x_char);
		port->icount.tx++;
		port->x_char = 0;
		goto out;
	}
	...
	*/
	...
	while (!uart_circ_empty(xmit) && count-- > 0) {
		if (rd_regl(port, S3C2440_UFSTAT) & ourport->info->tx_fifofull)
			break;

		wr_regb(port, S3C2440_UTXH, xmit->buf[xmit->tail]);
...
static unsigned int s3c24xx_serial_tx_empty(struct uart_port *port)
{
	...
	unsigned long ufstat = rd_regl(port, S3C2440_UFSTAT);
	unsigned long ufcon = rd_regl(port, S3C2440_UFCON);

	if (ufcon & S3C2440_UFCON_FIFOMODE) {
...
{
	unsigned int umstat = rd_regb(port,S3C2440_UMSTAT);

	if (umstat & S3C2440_UMSTAT_CTS)
...
static unsigned int s3c24xx_serial_getclk(struct uart_port *port,
					  ...
					  unsigned int baud)
{
	struct s3c2440_uartcfg *cfg = s3c24xx_port_to_cfg(port);
				       ...
				       struct termios *old)
{
	...
	case CS5:
		...
		ulcon = S3C2440_LCON_CS5;
		break;
	case CS6:
		dbg("config: 6bits/char\n");
		ulcon = S3C2440_LCON_CS6;
		break;
	case CS7:
		dbg("config: 7bits/char\n");
		ulcon = S3C2440_LCON_CS7;
		break;
	case CS8:
	default:
		dbg("config: 8bits/char\n");
		ulcon = S3C2440_LCON_CS8;
		break;
	}

	/* preserve original lcon IR settings */
	ulcon |= (cfg->ulcon & S3C2440_LCON_IRM);

	if (termios->c_cflag & CSTOPB)
		ulcon |= S3C2440_LCON_STOPB;

	umcon = (termios->c_cflag & CRTSCTS) ? S3C2440_UMCOM_AFC : 0;

	if (termios->c_cflag & PARENB) {
		if (termios->c_cflag & PARODD)
			ulcon |= S3C2440_LCON_PODD;
		else
			ulcon |= S3C2440_LCON_PEVEN;
	} else {
		ulcon |= S3C2440_LCON_PNONE;
	}
	...
	wr_regl(port, S3C2440_ULCON, ulcon);
	wr_regl(port, S3C2440_UBRDIV, quot);
	wr_regl(port, S3C2440_UMCON, umcon);

	dbg("uart: ulcon = 0x%08x, ucon = 0x%08x, ufcon = 0x%08x\n",
	    rd_regl(port, S3C2440_ULCON),
	    rd_regl(port, S3C2440_UCON),
	    rd_regl(port, S3C2440_UFCON));
	 ...
	 */
	port->read_status_mask = S3C2440_UERSTAT_OVERRUN;
	if (termios->c_iflag & INPCK)
		port->read_status_mask |= S3C2440_UERSTAT_FRAME | S3C2440_UERSTAT_PARITY;
	...
	if (termios->c_iflag & IGNPAR)
		port->ignore_status_mask |= S3C2440_UERSTAT_OVERRUN;
	if (termios->c_iflag & IGNBRK && termios->c_iflag & IGNPAR)
		port->ignore_status_mask |= S3C2440_UERSTAT_FRAME;
				    ...
				    struct s3c2440_uartcfg *cfg)
{
	...
	wr_regl(port, S3C2440_UCON,  cfg->ucon);
	wr_regl(port, S3C2440_ULCON, cfg->ulcon);

	/* reset both fifos */

	wr_regl(port, S3C2440_UFCON, cfg->ufcon | S3C2440_UFCON_RESETBOTH);
	wr_regl(port, S3C2440_UFCON, cfg->ufcon);
				    ...
				    struct platform_device *platdev)
{
	...
	port->membase	= (void __iomem *)(res->start - S3C2440_PA_UART);
	port->membase  += S3C2440_VA_UART;
...
{
	unsigned long ucon = rd_regl(port, S3C2440_UCON);
...
s3c24xx_serial_console_txrdy(struct uart_port *port, unsigned int ufcon)
{
	...
	utrstat = rd_regl(port, S3C2440_UTRSTAT);
	return (utrstat & S3C2440_UTRSTAT_TXE) ? 1 : 0;
}
...
{
	int i;
	unsigned int ufcon = rd_regl(cons_uart, S3C2440_UFCON);

	for (i = 0; i < count; i++) {
		while (!s3c24xx_serial_console_txrdy(cons_uart, ufcon))
			barrier();

		wr_regb(cons_uart, S3C2440_UTXH, s[i]);

		if (s[i] == '\n') {
			while (!s3c24xx_serial_console_txrdy(cons_uart, ufcon))
				barrier();

			wr_regb(cons_uart, S3C2440_UTXH, '\r');
		}
	}
}
			   ...
			   int *parity, int *bits)
{
	...
	ulcon  = rd_regl(port, S3C2440_ULCON);
	ucon   = rd_regl(port, S3C2440_UCON);
	ubrdiv = rd_regl(port, S3C2440_UBRDIV);
	...
	if ((ucon & 0xf) != 0) {
		...
		switch (ulcon & S3C2440_LCON_CSMASK) {
		case S3C2440_LCON_CS5:
			*bits = 5;
			break;
		case S3C2440_LCON_CS6:
			*bits = 6;
			break;
		case S3C2440_LCON_CS7:
			*bits = 7;
			break;
		default:
		case S3C2440_LCON_CS8:
			*bits = 8;
			break;
		}

		switch (ulcon & S3C2440_LCON_PMASK) {
		case S3C2440_LCON_PEVEN:
			*parity = 'e';
			break;

		case S3C2440_LCON_PODD:
			*parity = 'o';
			break;

		case S3C2440_LCON_PNONE:
		default:
			*parity = 'n';
		}
