 ...
 *
 * $Id: nand_base.c,v 1.2 2006/02/08 08:47:10 naushad Exp $
 *
			  ...
			  size_t * retlen, u_char * buf, u_char * oob_buf, struct nand_oobinfo *oobsel)
{
	...
	case NAND_ECC_HW8_512: /* Hardware ECC 8 byte / 512 byte data  */
		eccbytes = 8;
		break;
	case NAND_ECC_HW4_2048: /* Hardware ECC 4 byte / 2048 byte data  */
		eccbytes = 4;
		break;
	case NAND_ECC_NONE:
		compareecc = 0;
	...
	while (read < len) {
		...
		for (i = 0, j = 0, datidx = 0; i < this->eccsteps; i++, datidx += ecc) {
			...
			if (oob_buf && oobsel->useecc == MTD_NANDECC_PLACE) { 
				...
				p[i] = ecc_status;
				// Hack for hardware ecc,other wise yaffs will cry
				if(this->eccsteps == 1)
					p[i + 1] = ecc_status;
			}
		...
		if (oob_buf) {
			...
			case MTD_NANDECC_AUTOPLACE:
				...
				for (i = 0, j = 0; j < mtd->oobavail; i++) {
					...
					memcpy(&oob_buf[j+2], &oob_data[from], num);
					j+= num;
					if(num == 0 && from == 0) break;
				}
				oob += mtd->oobavail;
				break;
