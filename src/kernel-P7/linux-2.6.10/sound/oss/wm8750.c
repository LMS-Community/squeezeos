/*
 * Philips WM8750 mixer device driver
 *
 ...
 *
 * 2000-08-19	Erik Bunce	More inline w/ OSS API and WM8750 docs
...
/*
 * WM8750 L3 address and command types
 */
#define WM8750_L3ADDR		5
#define WM8750_DATA0		(WM8750_L3ADDR << 2 | 0)
#if !defined(CONFIG_UDA1344)
#define WM8750_DATA1		(WM8750_L3ADDR << 2 | 1)
#endif
#define WM8750_STATUS		(WM8750_L3ADDR << 2 | 2)
...
static void wm8750_sync(struct wm8750 *uda)
{
	char buf[24], *p = buf;
	...
	if (p != buf)
		l3_write(uda->l3_bus, WM8750_STATUS, buf, p - buf);
	...
	if (p != buf)
		l3_write(uda->l3_bus, WM8750_DATA0, buf, p - buf);
}
...
int wm8750_configure(struct wm8750 *uda, struct wm8750_cfg *conf)
{
	...
	if (ret == 0 && uda->active) {
		...
		l3_write(uda->l3_bus, WM8750_STATUS, &buf, 1);
	}

	return ret;
}

static int wm8750_update_direct(struct wm8750 *uda, int cmd, void *arg)
{
	...
	if (uda->active)
		l3_write(uda->l3_bus, WM8750_DATA0, &newreg, 1);
	return 0;
}

static int wm8750_update_indirect(struct wm8750 *uda, int cmd, void *arg)
{
...
static int wm8750_mixer_ioctl(struct wm8750 *uda, int cmd, void *arg)
{
	struct l3_gain gain;
...
printk("ioctl %x\n",cmd);
	...
	if (_IOC_DIR(cmd) & _IOC_WRITE) {
		...
		case SOUND_MIXER_VOLUME:
			uda->volume = val;
			uda->mod_cnt++;
			wm8750_update_direct(uda, L3_SET_VOLUME, &gain);
			break;

		case SOUND_MIXER_BASS:
			uda->bass = val;
			uda->mod_cnt++;
			wm8750_update_direct(uda, L3_SET_BASS, &gain);
			break;

		case SOUND_MIXER_TREBLE:
			uda->treble = val;
			uda->mod_cnt++;
			wm8750_update_direct(uda, L3_SET_TREBLE, &gain);
			break;
		...
		case SOUND_MIXER_LINE:
			...
			gain.channel = 1;
			uda->mod_cnt++;
			wm8750_update_indirect(uda, L3_SET_GAIN, &gain);
			break;
...
int wm8750_mixer_ctl(struct wm8750 *uda, int cmd, void *arg)
{
	...
	if (_IOC_TYPE(cmd) == 'M')
		ret = wm8750_mixer_ioctl(uda, cmd, arg);
...
int wm8750_open(struct wm8750 *uda)
{
	...
	l3_write(uda->l3_bus, WM8750_STATUS, buf, 2);
...
MODULE_DESCRIPTION("Wolfson WM8750 CODEC driver");
