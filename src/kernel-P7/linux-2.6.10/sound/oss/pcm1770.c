 ...
 *
 * 2000-08-19	Erik Bunce	More inline w/ OSS API and PCM1770 docs
...
/*
 * PCM1770 L3 address and command types
 */
#define PCM1770_L3ADDR		5
#define PCM1770_DATA0		(PCM1770_L3ADDR << 2 | 0)
#if !defined(CONFIG_UDA1344)
#define PCM1770_DATA1		(PCM1770_L3ADDR << 2 | 1)
#endif
#define PCM1770_STATUS		(PCM1770_L3ADDR << 2 | 2)
...
static void pcm1770_sync(struct pcm1770 *uda)
{
	char buf[24], *p = buf;
	...
	if (p != buf)
		l3_write(uda->l3_bus, PCM1770_STATUS, buf, p - buf);
	...
	if (p != buf)
		l3_write(uda->l3_bus, PCM1770_DATA0, buf, p - buf);
}
...
int pcm1770_configure(struct pcm1770 *uda, struct pcm1770_cfg *conf)
{
	...
	if (ret == 0 && uda->active) {
		...
		l3_write(uda->l3_bus, PCM1770_STATUS, &buf, 1);
	}

	return ret;
}

static int pcm1770_update_direct(struct pcm1770 *uda, int cmd, void *arg)
{
	...
	if (uda->active)
		l3_write(uda->l3_bus, PCM1770_DATA0, &newreg, 1);
	return 0;
}

static int pcm1770_update_indirect(struct pcm1770 *uda, int cmd, void *arg)
{
...
static int pcm1770_mixer_ioctl(struct pcm1770 *uda, int cmd, void *arg)
{
	struct l3_gain gain;
...
printk("ioctl %x\n",cmd);
	if (cmd == SOUND_MIXER_INFO) {
		...
		strncpy(mi.id, "PCM1770", sizeof(mi.id));
		strncpy(mi.name, "Philips PCM1770", sizeof(mi.name));
	...
	if (_IOC_DIR(cmd) & _IOC_WRITE) {
		...
		case SOUND_MIXER_VOLUME:
			uda->volume = val;
			uda->mod_cnt++;
			pcm1770_update_direct(uda, L3_SET_VOLUME, &gain);
			break;

		case SOUND_MIXER_BASS:
			uda->bass = val;
			uda->mod_cnt++;
			pcm1770_update_direct(uda, L3_SET_BASS, &gain);
			break;

		case SOUND_MIXER_TREBLE:
			uda->treble = val;
			uda->mod_cnt++;
			pcm1770_update_direct(uda, L3_SET_TREBLE, &gain);
			break;
		...
		case SOUND_MIXER_LINE:
			...
			gain.channel = 1;
			uda->mod_cnt++;
			pcm1770_update_indirect(uda, L3_SET_GAIN, &gain);
			break;
...
int pcm1770_mixer_ctl(struct pcm1770 *uda, int cmd, void *arg)
{
	...
	if (_IOC_TYPE(cmd) == 'M')
		ret = pcm1770_mixer_ioctl(uda, cmd, arg);
...
int pcm1770_open(struct pcm1770 *uda)
{
	...
	l3_write(uda->l3_bus, PCM1770_STATUS, buf, 2);
...
MODULE_DESCRIPTION("Philips PCM1770 CODEC driver");
